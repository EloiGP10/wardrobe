...
export function isWhiteForFlood(rgb, i) {
  const r = rgb[i];
  const g = rgb[i + 1];
  const b = rgb[i + 2];
  return Math.max(r, g, b) - Math.min(r, g, b) <= 40 && Math.max(r, g, b) >= 205;
}

// Background = white/light pixels 4-connected to the image border.
function floodFillWhite(rgb, w, h) {
  const bg = new Uint8Array(w * h);
  const queue = [];
  const push = (i) => { if (!bg[i] && isWhiteForFlood(rgb, i * 3)) { bg[i] = 1; queue.push(i); } };
  ...
  for (let qi = 0; qi < queue.length; qi++) {
    ...
    for (const d of dirs) {
      ...
      if (isWhiteForFlood(rgb, n * 3)) { bg[n] = 1; queue.push(n); }
    }
  }
  return bg;
}

// Opaque near-white rim pixels touching the background are leftover anti-aliased
// backdrop that would read as a thin white outline over the garment → drop them.
function removeWhiteRim(rgb, alpha, w, h) {
  if (w < 16 || h < 16) return;
  ...
  for (let y = 1; y < h - 1; y++) {
    for (let x = 1; x < w - 1; x++) {
      ...
      if (copy[i - 1] === 0 || copy[i + 1] === 0 || copy[i - w] === 0 || copy[i + w] === 0) {
        alpha[i] = 0;
      }
    }
  }
...
async function removeBackgroundFloodFill(buffer) {
  ...
  removeWhiteRim(data, alpha, info.width, info.height);
  return composeOutput(data, alpha, info.width, info.height);
}
...
function cropRgba(rgb, alpha, w, h) {
  ...
  const mw = Math.max(8, Math.round((x1 - x0 + 1) * 0.03));
  const mh = Math.max(8, Math.round((y1 - y0 + 1) * 0.03));
...
// Detect whether the image is a garment laid out on a plain light background:
// requires a wide white border ring. When true we can cut deterministically.
export async function isLightBackground(buffer) {
  const { data, info } = await readRgb(buffer);
  const w = info.width, h = info.height;
  if (w < 30 || h < 30) return false;
  let total = 0, white = 0;
  const ring = (count, check) => {
    let t = 0, wt = 0;
    while (count--) { const i = check(); t++; if (isWhiteForFlood(data, i * 3)) wt++; }
    total += t; white += wt;
  };
  ring(w + w, () => ((Math.random() < 0.5 ? 0 : h - 1) * w + Math.floor(Math.random() * w)) | 0);
  ring(h + h, () => (Math.floor(Math.random() * h) * w + (Math.random() < 0.5 ? 0 : w - 1)) | 0);
  return total > 0 && white / total >= 0.85;
}

function composeOutput(rgb, alpha, w, h) {
...
async function finalizeMask(buffer, alphaAi, skipChrome = false) {
  ...
  return composeOutput(rgb, alpha, w, h);
}

// ─── Public ───────────────────────────────────────────────────────────────────

export async function removeBackground(buffer) {
  if (process.env.MATTING_ENGINE === "native") return removeBackgroundFloodFill(buffer);
  if (await isLightBackground(buffer)) return removeBackgroundFloodFill(buffer);
  try {
